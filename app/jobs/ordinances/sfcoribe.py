...
class text_processing:
    mask_ordinance = r'PORTARIA\s*([Nn]\s*[\.&°:]*\s*\d*)'
    mask_cpf = r'\d*\s*[,*\.*]\s*\d*\s*[,*\.*]\s*\d*[_|–]\s*\d*|\d*\s*[,*\.*]\s*\d*\s*[,*\.*]\s*\d*/\d*[–|_]\s*\d*'
    mask_process = r'[Pp]rocesso\s*[SEMMATURH]*\s*[Nn]*[\.°:]*\s*(.+?),|Art'
    mask_deadline = r'v[áa]lida\s*[pelo\s*prazo\s*de]*\s*(.*)\s*anos'
    mask_area = r'[\d+,]* ha'
    ...
    issuer = 6321043
    issuer_name = 'São Félix do Coribe'
    ...
    def ls(text, search_date, dom_link):
        n_ordinance = re.findall(text_processing.mask_ordinance, text.upper())[0]
    ...
    def deadline_extension(text, search_date, dom_link):
        ...
        n_ordinance = re.findall(text_processing.mask_ordinance, text.upper())[0]
        changed_ordinance = re.findall(text_processing.mask_ordinance, text.upper())[1]
    ...
    def license_renewal(text, search_date, dom_link):
        mask_renov_type = r'RENOVA[GÇC][ÃA]O\s*D[AE]*\s*LICEN[GCÇ]A[\w\s]*(.*?),'
    ...
    def erratum(text, search_date, link):
        ...
        old_text = re.findall(r'ONDE SE LE:(.*)LEIA', text)[0][1:-1]
        new_text = re.findall(r'LEIA–SE:(.*)AVENIDA', text)[0][1:-1]
...
def _import_date(search_date):
    type_list = [
        ...
        r'RENOVA[GÇC][ÃA]O\s*D[AE]*\s*LICEN[GCÇ]A',
    ...
    files, links, folder = pdfutils.dom_sfcoribe(search_date)
