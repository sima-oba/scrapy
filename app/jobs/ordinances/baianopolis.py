...
class text_processing:
    ...
    mask_ordinance = r'[Portaria|PORTARIA]\s*[Gab|de|DE]*\s*[nN][°:2]*\s*(\d*/\d*)'
    mask_year = r'\d{4}'
    # mask_cpf = r'\d*\s*\.\s*\d*\s*\.\s*\d*[_|–]\d*|\d*\s*\.\s*\d*\s*\.\s*\d*/\d*[–|_]\d*'
    mask_cpf = r'\d*\s*[,*\.*]\s*\d*\s*[,*\.*]\s*\d*[_|–]\d*|\d*\s*[,*\.*]\s*\d*\s*[,*\.*]\s*\d*/\d*[–|_]\d*'
    mask_process = r'[Pp]rocesso\s*[SEMMARH]*\s*(.*)[,.]\s*RESOLVE'
    mask_deadline = r'v[ad]lida\s*pelo\s*per[if]odo\s*de\s*(\d*)|valida\s*por\s*(\d*)'
    mask_area = r'[\d+,]* ha'
    ...
    def ls(page_text, search_date, dom_link):
        n_ordinance = re.findall(text_processing.mask_ordinance, page_text)[0][1]
    ...
    def erratum(text, search_date, link):
        n_ordinance = re.findall(text_processing.mask_ordinance, text)[0]
        old_text = re.findall(r'ONDE SE L[EÉ]:(.*)LEIA', text.upper())[0][1:-1]
        new_text = re.findall(r'LEIA[.\s\w\-]*:(.*)\.P', text.upper())[0][1:-1]
...
def _import_date(search_date):
    ...
    files, links, folder = pdfutils.dom_baianopolis(search_date)
