...
class text_processing:
    ...
    mask_ordinance = r'Resolu[dcçagé]*o\s*(COMMAPT\s*[Nn]\s*[\.&°:]*\s*\d*)'
    mask_cpf = r'\d*\s*[,*\.*]\s*\d*\s*[,*\.*]\s*\d*[_|–]\s*\d*|\d*\s*[,*\.*]\s*\d*\s*[,*\.*]\s*\d*/\d*[–|_]\s*\d*'
    mask_process = r'PROCESSO\s*[SEMMATURH]*\s*[Nn]*[\.°:]*\s*(.+?)Art'
    mask_deadline = r'v[áa]lida\s*[pelo\s*prazo\s*de]*\s*(\d*)'
    
    
    issuer = 6320949
    issuer_name = 'Mansidão'
    ...
    def license_renewal(page_text, search_date, dom_link):
        mask_renov_type = r'EXPEDIR[:\s]*RENOVA[GC]AO\s*DE\s*LICEN[GCÇ]A\s*[DE\s](.*?),'
    ...
    def erratum(text, search_date, link):
        ...
        old_text = re.findall(r'[Oo]nde[\sse]*[LlIée]*[-se]*\s*(.*)[lL]eia', text)[0][:-1]
        new_text = re.findall(r'[Ll]eia-se[\s:]*(.*)\. ', text)[0][:-1]
...
def _import_date(search_date):
    ...
    type_list = [
        r'EXPEDIR\s*LICEN[GCÇ]*A\s*[AMBIENTAL \w]*\s*SIMPLIFICADA',
        r'EXPEDIR\s*LICEN[GCÇ]*A\s*[AMBIENTAL \w]*\s*UNIFICADA',
        r'EXPEDIR\s*LICEN[GCÇ]*A\s*[AMBIENTAL DE\w]*\s*OPERA',
        r'EXPEDIR\s*LICEN[GCÇ]*A\s*[AMBIENTAL DE\w]*\s*IMPLANTA',
        r'EXPEDIR\s*LICEN[GCÇ]*A\s*[AMBIENTAL DE\w]*\s*INSTALA',
        r'EXPEDIR\s*LICEN[GCÇ]*A\s*[AMBIENTAL DE\w]*\s*LOCALIZA',
        r'EXPEDIR\s*LICEN[GCÇ]*A\s*[AMBIENTAL \w]*\s*PR[EÉ]VIA',
        r'EXPEDIR\s*AUTORIZA[GÇC]*[AÃ]O\s*PARA\s*SUPRESS[AÃ]O\s*D[EA]\s*VEGETA[CÇG]*[AÃ]O\s*NATIVA',
        r'EXPEDIR\s*[A ]*PRORROGA[GC]AO\s*D[EO]\s*PRAZO\s*D[AE]\s*VALIDADE',
        r'EXPEDIR\s*[A ]*TRANSFER[ÊE]NCIA\s*DE\s*TITULARIDADE',
        r'RENOVA[GÇC][ÃA]O\s*DE*\s*LICEN[GCÇ]A',
        r'ERRATA\s*DA\s*PORTARIA',
        r'EXPEDIR\s*[A ]*DISPENSA\s*DE\s*LICEN',
        r'EXPEDIR\s*LICEN[GCÇ]*A\s*[AMBIENTAL DE\w]*\s*ALTERA',
    ...
    files, links, folder = pdfutils.dom_mansidao(search_date)
