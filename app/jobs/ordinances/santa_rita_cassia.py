...
class text_processing:
    mask_ordinance = r'PORTARIA\s*([SEMMATUR]*\s*N\s*[\.°:]*\s*\d*)'
    mask_cpf = r'\d*\s*[,*\.*]\s*\d*\s*[,*\.*]\s*\d*[_|–]\s*\d*|\d*\s*[,*\.*]\s*\d*\s*[,*\.*]\s*\d*/\d*[–|_]\s*\d*'
    mask_process = r'[Pp]rocesso\s*[SEMATUR]*\s*[Nn][\.°:]*\s*(.+?),'
    mask_deadline = r'v[áa]lida\s*[pelo\s*periodo\s*de]*\s*(\d*)'
    mask_area = r'[\d+,]* ha'
    mask_capture = r'captação [\w\s]*'
    mask_flow = r'vazão [\d.]*'
    
    issuer = 6321035
    issuer_name = 'Santa Rita de Cassia'
...
def _import_date(search_date):
    ...
    files, links, folder = pdfutils.dom_srcassia(search_date)
