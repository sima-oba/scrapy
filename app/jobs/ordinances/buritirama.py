...
class text_processing:
    mask_ordinance = r'N[¢¢]*2 (\d*/\d{4})'
    mask_cpf = r'\d*[,*\.*]\d*[,*\.*]\d*[_|–]\d*|\d*[,*\.*]\d*[,*\.*]\d*/\d*[–|_]\d*'
    mask_process = r'processo\s*[SEMMATURH]*\s*[Nn]*[\.°:]*\s*(.+?),'
    mask_deadline = r'v[áa]lida\s*[pelo\s*prazo\s*de]*\s*(\d*)'
    mask_area = r'[\d+,]* ha'
    ...
    # licença de operação
    def lo(text, search_date, dom_link):
        n_ordinance = re.findall(text_processing.mask_ordinance, text)[0]
        process = re.findall(text_processing.mask_process, text)[0].replace('–','-')
    ...
    def limp(text, search_date, dom_link):
        n_ordinance = re.findall(text_processing.mask_ordinance, text)[0]
        process = re.findall(text_processing.mask_process, text)[0].replace('–','-')
    ...
    def la(text, search_date, dom_link):
        n_ordinance = re.findall(text_processing.mask_ordinance, text)[0]
        process = re.findall(text_processing.mask_process, text)[0].replace('–','-')
    ...
    def linst(text, search_date, dom_link):
        n_ordinance = re.findall(text_processing.mask_ordinance, text)[0]
        process = re.findall(text_processing.mask_process, text)[0].replace('–','-')
    ...
    def ll(text, search_date, dom_link):
        n_ordinance = re.findall(text_processing.mask_ordinance, text)[0]
        process = re.findall(text_processing.mask_process, text)[0].replace('–','-')
    ...
    # licença prévia
    def preliminary_license(text, search_date, dom_link):
        n_ordinance = re.findall(text_processing.mask_ordinance, text)[0]
        process = re.findall(text_processing.mask_process, text)[0].replace('–','-')
    ...
    def asv(text, search_date, dom_link):
        n_ordinance = re.findall(text_processing.mask_ordinance, text)[0]
        process = re.findall(text_processing.mask_process, text)[0].replace('–','-')
    ...
    def condition_review(text, search_date, link):
        n_ordinance = re.findall(text_processing.mask_ordinance, text)[0]
        process = re.findall(text_processing.mask_process, text)[0]
        doc_number = re.findall(text_processing.mask_cpf, text)[0]
        origin_ordinance = re.findall(text_processing.mask_ordinance, text)[1]
    ...
    def erratum(text, search_date, link):
        ...
        n_ordinance = re.findall(mask_ordinance, text[0])[0]
        old_text = re.findall(f'PORTARIA .*{mask_ordinance}(.*)Gabinete',text[0])[0][1:-1]
        new_text = re.findall(f'PORTARIA .*{mask_ordinance}(.*)Gabinete',text[1])[0][1:-1]
...
def _import_date(search_date):
    type_list = [
        ...
        r'CONCEDE[R]*\s*LICEN[GCÇ]*A\s*[AMBIENTAL DE]*\s*OPERA',
        r'CONCEDE[R]*\s*LICEN[GCÇ]*A\s*[AMBIENTAL DE]*\s*IMPLANTA',
        r'CONCEDE[R]*\s*LICEN[GCÇ]*A\s*[AMBIENTAL DE]*\s*INSTALA',
        r'CONCEDE[R]*\s*LICEN[GCÇ]*A\s*[AMBIENTAL DE]*\s*LOCALIZA',
        r'CONCEDE[R]*\s*LICEN[GCÇ]*A\s*[AMBIENTAL DE]*\s*ALTERA',
        r'CONCEDE[R]*\s*LICEN[GCÇ]*A\s*[AMBIENTAL]*\s*PR',
    ...
    type2function = {
        ...
        type_list[13]: 'erratum'
    }

    files, links, folder = pdfutils.dom_buritima(search_date)
        ...
        for i in range(len(pages)):
            ...
            for ord_type in type_list:
                ...
                if len(re.findall(ord_type, ordinance_text.upper())) > 0:
                    ...
                    # erratas para esse município estão divididas em duas páginas, então a função de processá-la 
                    # espera receber um vetor com as duas páginas
                    if(type2function[ord_type] == 'erratum'):
                        ordinance_text = [pages[i], pages[i+1]]
