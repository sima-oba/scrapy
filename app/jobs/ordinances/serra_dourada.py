...
class text_processing: 
    ...
    def erratum(text, search_date, link):
        mask_ordinance = r'PORTARIA N° (\d*).*DE (\d{4})'

        n_ordinance = re.findall(mask_ordinance, text)
        n_ordinance = f'{n_ordinance[0][0]}/{n_ordinance[0][1]}'

        # origin ordinance
        mask_ordinance = r'PORTARIA N° (\d*/\d{4})'
        origin_ordinance = re.findall(mask_ordinance, text.upper())

        old_text = re.findall(r'Onde[\sse]*[LI][ée][-se]*\s*:\s*(.*)\s*Leia', text)[0][:-1]
        new_text = re.findall(r'Leia-se\s*:\s*(.*)\s*Art. 2', text)[0][:-1]
...
def _import_date(search_date):
    ...
    files, links, folder = pdfutils.dom_serra_dourada(search_date)
