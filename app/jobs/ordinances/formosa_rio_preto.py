...
class text_processing:
    ...
    mask_process = r'\d*\s*.\s*\d*\s*.\s*TEC\s*.\s*\w*\s*.\s*\d*'
    mask_deadline = r'(\d{2}?)[(\w) ]*ano[s]*|v[aá]lida por (\d*)'
    mask_cpf = r'\d{3}\.\d{3}\.\d{3}\s*–\s*\d{2}|\d*[\.\s]\d*[\.\s]\d*/\d*–\s*\d*'
    issuer_name = 'Formosa do Rio Preto'
    issuer = 3451478
    ...
    def tt(page_text, search_date, link):
        ...
        old_doc = doc_number[1].replace('–','-').replace(' ', '')
        new_doc = doc_number[2].replace('–','-').replace(' ', '')
...
def _import_date(search_date):
    type_list = [
        r'CONCEDER[R]*\s*[A _]*LICEN[CG]A[\w\s]*SIMPLIFICADA',
        r'CONCEDER[R]*\s*[A _]*LICEN[CG]A[\w\s]*UNIFICADA',
        r'CONCEDER[R]*\s*[A _]*LICEN[CG]A[\w\s]*LOCALIZA[GC]AO',
        r'CONCEDER[R]*\s*[A _]*LICEN[CG]A[\w\s]*INSTALA[GC]AO',
        r'CONCEDER[R]*\s*[A _]*LICEN[CG]A[\w\s]*IMPLANTA[GC]AO',
        r'CONCEDER[R]*\s*[A _]*LICEN[CG]A[\w\s]*OPERA[GC]AO',
        r'CONCEDER[R]*\s*[A _]*LICEN[CG]A[\w\s]*PR[ÉE]VIA',
        r'CONCEDER[R]*\s*.*APROVA[CG]AO\s*DE\s*LOCALIZA[GC]AO\s*DE\s*RESERVA\s*LEGAL',
        r'CONCEDER[R]*\s*.*AUTORIZA[CG¢ADO]*\s*PARA\s*SUPRESSAO\s*DA\s*VEGETA[CG¢]*AO[\sNATIVA]*',
        r'CONCEDER[R]*\s*.*DISPENSA\s*DE\s*LICENCA\s*AMBIENTAL',
        r'CONCEDER[R]*\s*RENOVA[GC][AÃ]O\s*DE\s*LICEN[GCÇ]A',
        r'TRANSFERIR.*TITULARIDADE',
        r'ALTERAR|RETIFICA\s*.*\s*RAZ[AÃ]O\s*SOCIAL',
        r'REVOGAR[R]*',
        r'CONCEDER[R]*\s*[A _]*PRORROGA[GC]AO\s*D[EO]\s*PRAZO\s*D[AE]\s*VALIDADE',
    ...
    type2function = {
        ...
        type_list[15]: 'condition_review',
        type_list[16]: 'erratum'
    }

    files, folder, links = pdfutils.dom_formosa_rio_preto(search_date)
